...
// Prawidłowa kolejność obrotu samochodu
...
export default function VehicleDetailsPage({ params }: { params: Promise<{ vehicleId: string }> }) {
  ...
  const [formData, setFormData] = useState({
    brand: '', model: '', plate: '', vin: '', firstRegistrationDate: '', currentRegistrationDate: ''
  });
    ...
    const fetchVehicle = async () => {
      try {
        ...
        if (docSnap.exists()) {
          ...
          setVehicle(data);
          setFormData({
            brand: data.brand || '', model: data.model || '', plate: data.plate || '',
            vin: data.vin || '', firstRegistrationDate: data.firstRegistrationDate || '', currentRegistrationDate: data.currentRegistrationDate || ''
          });
        }
      } catch (error) {
        console.error(error);
  ...
  const togglePhotoSelection = (stepId: string) => {
    setSelectedPhotos(prev => prev.includes(stepId) ? prev.filter(id => id !== stepId) : [...prev, stepId]);
  };

  const handleDeletePhoto = async (stepId: string, url: string) => {
    if (!confirm('Czy na pewno chcesz usunąć to zdjęcie?')) return;
    try {
      try { await deleteObject(ref(storage, url)); } catch (e) { /* ciche ignorowanie błędu Storage */ }
  ...
  const handleDeleteKartoteka = async () => {
    if (!confirm('UWAGA! Czy na pewno chcesz usunąć tę kartotekę i wszystkie zdjęcia?')) return;
    setIsSaving(true);
    try {
      if (vehicle.photos) {
        for (const url of Object.values(vehicle.photos)) {
          if (url !== 'loading') {
            try { await deleteObject(ref(storage, url as string)); } catch (e) {}
          }
        }
      }
  ...
  const handleRemoveBackgrounds = async () => {
    if (selectedPhotos.length === 0) return alert("Wybierz najpierw zdjęcia.");
    ...
    try {
      ...
      for (let i = 0; i < selectedPhotos.length; i++) {
        ...
        if (!contentType || !contentType.includes("application/json")) {
           throw new Error(`Brak endpointu API. Sprawdź serwer.`);
        }

        const data = await res.json();
        if (!res.ok || !data.success) throw new Error(data.error || 'Błąd API remove.bg');
      ...
      setBgProgress(prev => ({ ...prev, status: 'Aktualizacja bazy...' }));
      await updateDoc(doc(db, 'vehicles', resolvedParams.vehicleId), { photos: updatedPhotos });
      setVehicle({ ...vehicle, photos: updatedPhotos });
      setSelectedPhotos([]); 
      alert("Tło usunięte!");
  ...
  // Logika rotacji 360°
  // Wyciągamy i sortujemy tylko dostępne zdjęcia zewnętrzne
  ...
  const handlePointerMove = useCallback((e: React.PointerEvent) => {
    ...
    // Zmieniamy klatkę co 30 pikseli przeciągnięcia
      ...
      {show360 && available360Photos.length > 0 && (
        <div className="fixed inset-0 z-50 bg-slate-950 flex flex-col">
          ...
          <div className="p-6 flex justify-between items-center z-10">
            <div>
              <h2 className="text-2xl font-bold text-white flex items-center gap-2"><Sparkles className="w-6 h-6 text-indigo-400" /> Studio 360°</h2>
              <p className="text-slate-400 text-sm">{vehicle.brand} {vehicle.model}</p>
            </div>
            <button onClick={() => setShow360(false)} className="p-3 bg-slate-800 hover:bg-slate-700 rounded-full transition-colors border border-slate-700">
              <X className="w-6 h-6" />
            </button>
          </div>
          ...
          <div 
            className="flex-1 relative overflow-hidden select-none touch-none bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 flex items-center justify-center cursor-grab active:cursor-grabbing"
          ...
          >
            ...
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 translate-y-[20%] w-[80%] max-w-3xl h-32 bg-black/50 blur-3xl rounded-[100%] pointer-events-none"></div>

            <img 
              src={available360Photos[currentFrame]} 
              alt="Widok 360"
              className="relative z-10 w-full max-w-5xl max-h-[70vh] object-contain drop-shadow-[0_20px_25px_rgba(0,0,0,0.5)] pointer-events-none"
              draggable="false"
            />

            {/* Wskaźnik przeciągania */}
            <div className="absolute bottom-12 left-1/2 -translate-x-1/2 flex flex-col items-center animate-pulse opacity-50 pointer-events-none">
              <MoveHorizontal className="w-8 h-8 text-white mb-2" />
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 bg-slate-900 p-6 rounded-2xl border border-slate-800">
          ...
          <div className="flex flex-wrap gap-2 md:gap-3 w-full xl:w-auto">
            ...
            <button 
              ...
              disabled={available360Photos.length === 0}
              className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg text-sm font-bold flex items-center gap-2 transition-all disabled:opacity-50 shadow-[0_0_15px_rgba(79,70,229,0.4)]"
            >
              <Eye className="w-4 h-4" /> Podgląd 360°
            </button>

            <Link href={`/session/${resolvedParams.vehicleId}`} className="px-4 py-2 bg-emerald-600/20 text-emerald-400 hover:bg-emerald-600/30 border border-emerald-500/30 rounded-lg text-sm font-semibold flex items-center gap-2 transition-all">
            ...
            <button onClick={handleDeleteKartoteka} disabled={isSaving} className="bg-rose-500/10 hover:bg-rose-500/20 text-rose-500 border border-rose-500/20 px-4 py-2 rounded-lg text-sm font-semibold flex items-center gap-2 transition-all disabled:opacity-50 ml-auto xl:ml-2">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1 bg-slate-900 p-6 rounded-2xl border border-slate-800 h-fit space-y-4">
            ...
            {[
              { label: 'Marka', name: 'brand' }, { label: 'Model', name: 'model' },
              { label: 'Nr Rejestracyjny', name: 'plate' }, { label: 'Numer VIN', name: 'vin' },
              { label: 'Pierwsza Rejestracja', name: 'firstRegistrationDate' }, { label: 'Data Wydania DR', name: 'currentRegistrationDate' }
            ].map(field => (
              <div key={field.name} className="flex flex-col">
                <label className="text-xs text-slate-400 uppercase tracking-wider mb-1 font-semibold">{field.label}</label>
                {isEditing ? (
                  <input type="text" name={field.name} value={formData[field.name as keyof typeof formData]} onChange={handleInputChange} className="bg-slate-950 border border-slate-700 rounded-lg p-2.5 text-white focus:border-emerald-500" />
                ) : (
                  <div className="bg-slate-950/50 border border-slate-800/50 rounded-lg p-2.5 text-slate-200 font-medium">{vehicle[field.name] || 'Brak'}</div>
                )}
              </div>
            ))}
          </div>
                      ...
                      {!isLoading && (
                        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity p-2 flex flex-col justify-between">
                          <div className="flex justify-between items-start">
                            <button onClick={() => togglePhotoSelection(stepId)} className="text-white p-1 hover:text-indigo-400">
                          ...
                          <div className="flex justify-center">
                             <span className="text-white text-xs font-bold uppercase bg-black/70 px-3 py-1 rounded-md">{stepId.replace('ext_', '').replace('_', ' ')}</span>
                          </div>
                        </div>
                      )}
                    </div>
